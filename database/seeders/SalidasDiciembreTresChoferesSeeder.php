...
class SalidasDiciembreTresChoferesSeeder extends Seeder
{
    /**
     * Registrar salidas de diciembre 2025 con LOS 3 CHOFERES por día
     * Excluye domingos, 24, 25 y 31 de diciembre
     */
    public function run(): void
    {
        // Choferes con sus vehículos asignados (TODOS salen cada día)
        $choferes = [
            [
                'nombre' => 'deybi aguilar',
                'vehiculo_placa' => '1234ABC',
                'distribuidor' => 'Antonio Rodriguez'
            ],
            [
                'nombre' => 'Sergio Aguilar',
                'vehiculo_placa' => '5678DEF',
                'distribuidor' => 'joel buendia'
            ],
            [
                'nombre' => 'Jasmani Aguilar',
                'vehiculo_placa' => '9012GHI',
                'distribuidor' => 'nano alvarez'
            ],
        ];
        ...
        // Generar fechas (excluir domingos, 24, 25, 31)
        $fechas = [];
        for ($dia = 1; $dia <= 31; $dia++) {
            $fecha = Carbon::create(2025, 12, $dia);
            ...
            // Excluir 24, 25, 31 de diciembre
            if (in_array($dia, [24, 25, 31])) {
                continue;
            }
        ...
        $this->command->info("🚚 Iniciando registro de salidas de diciembre 2025 (3 CHOFERES POR DÍA)...");
        $this->command->info("📅 Días laborables: " . count($fechas) . " días");
        $this->command->info("🚗 Choferes por día: 3");
        ...
        try {
            foreach ($fechas as $fecha) {
                $this->command->info("");
                $this->command->info("📆 {$fecha->format('d/m/Y')} ({$fecha->locale('es')->isoFormat('dddd')}):");

                // LOS 3 CHOFERES salen el mismo día
                foreach ($choferes as $chofer) {
                    ...
                    $retornoBotellones = rand(0, min(15, (int)($botellones * 0.3)));
                    ...
                    DB::table('control_salidas_productos')->insert([
                        'nombre_distribuidor' => $chofer['distribuidor'],
                        ...
                        'observaciones' => sprintf(
                            'Despacho diario - Vendidos: %d unidades - Retornos: %d',
            ...
            $this->command->info("  • Días trabajados: " . count($fechas));
            $this->command->info("  • Salidas por día: 3 (los 3 choferes)");
            $this->command->info("  • Choferes: deybi aguilar, Sergio Aguilar, Jasmani Aguilar");
            $this->command->info("  • Días excluidos: Domingos, 24, 25 y 31 de diciembre");
            $this->command->info("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
